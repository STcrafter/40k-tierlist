...
В расчёты тирлиста, отчётов и веб-данных попадают только актуальные юниты стоимостью **не более 2000 очков**. Юниты с пометкой `[Legends]` исключаются как отдельный корпус правил. Более дорогие модели по-прежнему разбираются адаптером, но не участвуют в нормализации, перцентилях и тирах.

Урон и живучесть в тирлисте считаются на 100 очков стоимости. Для живучести используется ограниченная стоимостная шкала `100 / (1 + takenPer100)`: очень большой запас HP титана не превращается в бесконечное преимущество, но реальное преимущество прочности внутри одного бюджета сохраняется.
